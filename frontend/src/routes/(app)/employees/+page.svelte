...
<div class="p-6 md:p-8">
  <div class="mb-6 flex items-center justify-between gap-4">
    <div>
      ...
      <p class="mt-1 text-sm text-foreground-muted">{meta ? `${meta.total} empleado(s)` : 'Cargando...'}</p>
    </div>
  ...
  <Card class="overflow-hidden p-0">
    ...
    {:else if employees.length === 0}
      <div class="flex flex-col items-center justify-center py-16"><p class="text-sm text-foreground-muted">No se encontraron empleados.</p></div>
    {:else}
          ...
          <tbody class="divide-y divide-border">
            {#each employees as emp (emp.id)}
              <tr class="hover:bg-surface-muted">
                ...
                <td class="px-4 py-3 text-foreground-muted">{emp.hire_date ? new Date(emp.hire_date).toLocaleDateString('es-ES') : '—'}</td>
              </tr>
            {/each}
          </tbody>
        </table>
